    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Monitor billing status and transaction history for all tenants.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <Card className="border-border bg-card/50">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">$1,820</div>
            <p className="text-xs text-muted-foreground mt-1">7 tenants with outstanding balance</p>
          </CardContent>
        </Card>
        <Card className="border-border bg-card/50">
          <CardHeader className="pb-2">
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <CardDescription className="text-muted-foreground">Review and manage all billing documents.</CardDescription>
            </div>
            <div className="flex gap-2">
              <div className="relative w-64">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input placeholder="Search invoice or tenant..." className="pl-10 bg-background border-input" />
              </div>
                ...
                {invoices.map((inv) => (
                  <tr key={inv.id} className="group hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <div className="font-semibold text-foreground">{inv.tenant}</div>
                        <div className="text-xs text-muted-foreground">{inv.owner}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-muted text-muted-foreground ml-1">
