...
import { useFirebase, useCollection, useDoc, useMemoFirebase, setDocumentNonBlocking, deleteDocumentNonBlocking } from '@/firebase';
import { collection, doc, serverTimestamp, Timestamp } from 'firebase/firestore';
import type { Client, CalendarEvent } from '@/lib/types';
...
export default function EditSessionPage() {
  ...
  const eventDocRef = useMemoFirebase(
    () => (firestore && tenantId && eventId ? doc(firestore, `tenants/${tenantId}/events`, eventId) : null),
  ...
  useEffect(() => {
    if (eventData) {
        const start = (eventData.start as Timestamp).toDate();
        const end = (eventData.end as Timestamp).toDate();
  ...
  const onSubmit = async (data: SessionFormData) => {
    ...
    try {
        await setDocumentNonBlocking(eventDocRef, eventDataToSave, { merge: true });
        toast({ variant: 'success', title: "Sesión Actualizada", description: `La sesión "${data.title}" ha sido guardada.` });
        router.push('/schedule');
    } catch (e: any) {
        toast({ variant: 'destructive', title: "Error", description: "No se pudo actualizar la sesión." });
    }
  };
  ...
  const handleDelete = async () => {
    if (!eventDocRef) return;
    try {
        await deleteDocumentNonBlocking(eventDocRef);
        toast({ variant: 'success', title: "Sesión Eliminada", description: "La sesión ha sido eliminada de la agenda." });
        router.push('/schedule');
    } catch(e: any) {
      ...
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                ...
                <span>/</span>
                <span className="truncate max-w-xs">{eventData?.title || 'Editar Sesión'}</span>
            </div>
            <h1 className="text-3xl font-bold font-headline">Editar Sesión</h1>
        </div>
        <div className="flex items-center gap-2">
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <Button variant="destructive-outline"><Trash2 className="mr-2 h-4 w-4" /> Eliminar</Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  <AlertDialogTitle>¿Estás seguro?</AlertDialogTitle>
                  <AlertDialogDescription>
                    Esta acción no se puede deshacer. Se eliminará permanentemente la sesión de la agenda.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
