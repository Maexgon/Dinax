...
interface ServicePlanFormProps {
    existingPlan?: ServicePlan | null;
}

export default function ServicePlanForm({ existingPlan }: ServicePlanFormProps) {
  const { t } = useLanguage();
  ...
  const onSubmit = async (data: ServicePlanFormData) => {
    if (!firestore || !user) {
        toast({ variant: "destructive", title: t.services.errorSaving, description: "Not authenticated." });
        return;
    }
    
    const planId = existingPlan?.id || doc(collection(firestore, `tenants/${user.uid}/services`)).id;
    const planRef = doc(firestore, `tenants/${user.uid}/services`, planId);
    
    const planToSave = { ...data, id: planId };

    try {
        await setDocumentNonBlocking(planRef, planToSave, { merge: true });
        toast({ variant: 'success', title: t.services.planSaved, description: t.services.planSavedSuccess });
        router.push('/services');
    } catch(e: any) {
        toast({ variant: "destructive", title: t.services.errorSaving, description: e.message });
    }
  };
        ...
        <div className="lg:col-span-5 xl:col-span-4">
          <div className="sticky top-6 space-y-4">
                ...
                <div className="p-6 md:p-8 flex flex-col items-center text-center">
                    ...
                    <div className="flex flex-col items-center mb-1">
                        <div className="flex items-baseline gap-2">
                          ...
                          <span className="text-muted-foreground font-medium">/ {(planFrequencies.find(f => f.value === watchData.frequency)?.label || t.services.monthly).toLowerCase()}</span>
                        </div>
                         {watchData.hasPromo && Number(watchData.price) > 0 && (
                    ...
                    {watchData.hasPromo && watchData.promoValue && (
                        <div className="mb-8 mt-2">
                             <Badge variant="secondary" className="bg-orange-100 text-orange-600 dark:bg-orange-900/30 dark:text-orange-400">
                                ...
                                {watchData.promoValue}{watchData.promoType === 'percentage' ? '%' : currencySymbol} OFF {watchData.promoDuration === 'custom' && watchData.promoMonths ? `x ${watchData.promoMonths} ${t.services.months}`: (promoDurations.find(p=>p.value === watchData.promoDuration)?.label || '')}
                            </Badge>
                        </div>
                    )}
            ...
            <div className="mt-4 flex flex-col gap-3">
                ...
                <Button type="submit" className="w-full bg-foreground text-background hover:bg-foreground/80 font-bold shadow-xl" disabled={isSubmitting}>
                     {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Rocket className="mr-2"/>}
                     {existingPlan ? t.clientDetail.saveChanges : t.services.publishPlan}
                </Button>
            </div>
          </div>
        </div>
